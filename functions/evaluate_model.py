...
def evaluate(model1, model2, df, start_date, end_date, horizon_format='days', last_x=5, years=False, months=False, weeks=True):
    ...
    - horizon_format: Format of the forecast horizon ('days' or 'hours').
    ...
    for current_date in date_range:
        ...
        evaluation_model1 = evaluate_and_append(evaluation_model1, pred_model1, df, horizon_format)

        # Evaluate model2
        pred_model2 = model2['function'](df_before, date_str=date_str)
        evaluation_model2 = evaluate_and_append(evaluation_model2, pred_model2, df, horizon_format)
...
def evaluate_and_append(evaluation_df, pred, df, horizon_format):
    '''
    Append predictions to the evaluation DataFrame with appropriate horizon handling.
    - evaluation_df: DataFrame to append to.
    - pred: Predictions DataFrame.
    - df: Original data DataFrame.
    - horizon_format: 'days' or 'hours'.
    '''
